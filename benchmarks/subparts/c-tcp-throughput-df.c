...
int server(char * port, int pings, int size) {
...
int client(char *addr, char *port, int pings, int size) {
...
int usage(int argc, char** argv) {
  printf("usage: %s <CLIENT|SERVER> <address> <port> <number of pings> <size of a ping>\n", argv[0]);
  return -1;
}
...
int main(int argc, char** argv) {
  ...
  int pings = 0;
  int size  = 0;
  sscanf(argv[4], "%d", &pings);
  sscanf(argv[5], "%d", &size);
  if (strcmp(argv[1], "CLIENT") == 0) {
    return client(argv[2], argv[3], pings, size);
  } else if (strcmp(argv[1], "SERVER") == 0) {
    return server(argv[3], pings, size);
  }
}
